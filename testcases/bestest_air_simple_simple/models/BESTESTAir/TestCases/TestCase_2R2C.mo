...
model TestCase_2R2C "Testcase model with ideal airflow"
  extends Modelica.Icons.Example;

  BaseClasses.Thermostat_T con "Thermostat controller"
    annotation (Placement(transformation(extent={{-80,-10},{-60,10}})));
  BESTESTAir.BaseClasses.FanCoilUnit_T fcu "Fan coil unit"
    annotation (Placement(visible = true, transformation(origin = {-4, 0}, extent = {{-20, -8}, {0, 20}}, rotation = 0)));
  BESTESTAir.BaseClasses.SingleZone2R2C zon
    annotation (Placement(visible = true, transformation(origin = {6, 8}, extent = {{32, -12}, {52, 8}}, rotation = 0)));
equation
  connect(con.TSup, fcu.TSup) annotation (
    Line(points={{-59,6},{-44,6},{-44,9},{-25,9}},          color = {0, 0, 127}));
  connect(con.yFan, fcu.uFan) annotation (
    Line(points={{-59,0},{-44,0},{-44,3},{-25,3}},          color = {0, 0, 127}));
  connect(fcu.supplyAir, zon.supplyAir) annotation (Line(points={{-4, 14},{4,14},{4,8},{38,8}}, color={0,127,255}));
  connect(fcu.returnAir, zon.returnAir) annotation (Line(points={{-4,-6},{
          4,-6},{4,4},{38,4}}, color={0,127,255}));
  connect(zon.y, con.TZon) annotation (Line(points={{58,13},{86,13},{86,-42},
          {-94,-42},{-94,0},{-82,0}}, color={0,0,127}));
