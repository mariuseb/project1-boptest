...
model RealOccupancy
  extends BuildingControl(
    ...
    infiltration(ach=0.2, infiltr(use_m_flow_in=false)),
    scale_factor(k=4),
    rad(dp_nominal=0),
    conPIDcoil(
      yMax=1,
    ...
    metHeat(k=120/AFlo),
    weaDat(filNam=ModelicaServices.ExternalReferences.loadResource("OU44Emulator/Resources/Climate/DNK_Copenhagen.061800_IWEC.mos"), pAtmSou=
         Buildings.BoundaryConditions.Types.DataSource.Parameter),
    senCO2(warnAboutOnePortConnection=false),
    ahu(
      resEx(dp_nominal=1, deltaM=1),
      hex(dp1_nominal=500, dp2_nominal=500),
      hex1(dp1_nominal=1, dp2_nominal=250)));
  ...
  Buildings.Utilities.IO.SignalExchange.Read reaCO2Gai(
    description="Zone CO2 gain",
    KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.None,
    y(unit="None")) "Read CO2 gain"
    annotation (Placement(transformation(extent={{0,154},{-14,168}})));

  Buildings.Utilities.IO.SignalExchange.Read reaPVent(
    y(unit="W"),
    KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.None,
    description="Thermal power of AHU") "Thermal power of AHU"
    annotation (Placement(transformation(extent={{164,-142},{186,-120}})));

  Buildings.Utilities.IO.SignalExchange.Read reaPRad(
    y(unit="W"),
    KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.None,
    description="Thermal power of AHU") "Thermal power of radiator"
    annotation (Placement(transformation(extent={{176,-208},{198,-186}})));
...
equation
  ...
  connect(occupancy.y[1], reaOcc.u)
    annotation (Line(points={{-169,140},{-149.6,140}}, color={0,0,127}));
  ...
  connect(reaCO2Gai.u, scale_factor.y) annotation (Line(points={{1.4,161},{26,
          161},{26,112},{-47.2,112}}, color={0,0,127}));
  connect(reaPVent.u, Qh_coil.u) annotation (Line(points={{161.8,-131},{150,
          -131},{150,-126},{104,-126},{104,-124},{74,-124},{74,-134},{72,-134},
          {72,-140},{78,-140}},                            color={0,0,127}));
  connect(reaPRad.u, Qh_rad.u) annotation (Line(points={{173.8,-197},{78,-197},
          {78,-172}}, color={0,0,127}));
