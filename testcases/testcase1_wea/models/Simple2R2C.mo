...
model Simple2R2C
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor Ria(R = 0.001) annotation (
  ...
  Modelica.Blocks.Sources.CombiTimeTable TSetHea(extrapolation = Modelica.Blocks.Types.Extrapolation.Periodic, smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, table = [0, TSetHeaUno; occSta, TSetHeaOcc; occEnd, TSetHeaUno; 24*3600, TSetHeaUno]) annotation (
    Placement(visible = true, transformation(origin={-48,-72},    extent = {{-100, 30}, {-80, 50}}, rotation = 0)));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow preSol
    annotation (Placement(transformation(extent={{40,20},{60,40}})));
  Modelica.Blocks.Math.Gain Aw(k= 5)
    annotation (Placement(transformation(extent={{-14,20},{6,40}})));
equation
  connect(Ci.port, senTZone.port)
    annotation (Line(points={{80,0},{120,0}}, color={191,0,0}));
  ...
  connect(preHeat.port, Ci.port) annotation (Line(points={{28,-30},{80,-30},{80,
          -26},{81,-26},{81,-8},{82.5,-8},{82.5,0},{80,0}}, color={191,0,0}));
  ...
  connect(TSetHea.y[1], oveTset.u) annotation (
    Line(points={{-127,-32},{-118,-32},{-118,-30},{-108,-30}},
                                              color = {0, 0, 127}));
  connect(Aw.y, preSol.Q_flow) annotation (Line(points={{7,30},{40,30}},
                                    color={0,0,127}));
  connect(Aw.u, weaBus.HGloHor) annotation (Line(points={{-16,30},{-20,30},{-20,
          72},{-49,72}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(preSol.port, Ci.port)
    annotation (Line(points={{60,30},{70,30},{70,0},{80,0}}, color={191,0,0}));
  annotation (
